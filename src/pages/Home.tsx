    ...
    <div className="space-y-5">
      ...
      <div className="hide-scroll flex gap-2 overflow-x-auto pb-1">
        {(overview.data?.indices || []).slice(0, 4).map((ix) => (
          <div key={ix.symbol} className="card min-w-[148px] px-3 py-2.5">
        ...
        {overview.loading &&
          Array.from({ length: 4 }).map((_, i) => <div key={i} className="skel h-[62px] min-w-[148px] rounded-2xl" />)}
      </div>

      <div>
        <div className="hide-scroll -mx-1 flex gap-1 overflow-x-auto px-1 pb-2">
          {TABS.map((t) => {
            const Icon = t.icon;
        ...
        {tab === "favourite" && !favourites.length && (
          <EmptyState
            ...
            action={
              <PrimaryButton onClick={onSearch}>Search for a stock</PrimaryButton>
            }
          />
        )}
